...
script {
	...
	fun reveal_tokens(
		...
		let old_token_names = vector<String> [
				std::string::utf8(b"Kreacher #651"),
		];

		let new_token_numbers = vector<u64>[
			123,
		];

		vector::reverse(&mut old_token_names);
		vector::reverse(&mut new_token_numbers);
		while (vector::length(&old_token_names) > 0) {
			let old_token_name = vector::pop_back(&mut old_token_names);
			let new_token_number = vector::pop_back(&mut new_token_numbers);
			pond::swap_reveal::new_swap_token(
				revealer,
				old_token_name,
				new_token_number,
				TESTNET_CREATOR
